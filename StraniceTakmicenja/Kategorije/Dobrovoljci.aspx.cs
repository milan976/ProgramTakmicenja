...
{
    public partial class Dobrovoljci : Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                BindEkipePoKategoriji("ДВД");
            }
        }

        protected void ddlFilterEkipaDvd_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (ddlFilterEkipaDvd.SelectedValue == "0")
            {
                lblPocetniBodoviDobrovoljci.Text = "Почетни бодови: 100.00";
                OcistiFormu();
                ResetujStatistiku();
                return;
            }
            ...
            {
                cmd.Parameters.AddWithValue("@EkipaID", ddlFilterEkipaDvd.SelectedValue);

                con.Open();
                object result = cmd.ExecuteScalar();
                if (result != null)
                {
                    lblPocetniBodoviDobrovoljci.Text = $"Почетни бодови: {result.ToString()}";
                }
                else
                {
                    lblPocetniBodoviDobrovoljci.Text = "Почетни бодови: 100.00";
            ...
            UcitajPodatkeZaEkipuDobrovoljci(ddlFilterEkipaDvd.SelectedValue);

            ScriptManager.RegisterStartupScript(this, this.GetType(), "initDobrovoljci",
                "setTimeout(function() { initDobrovoljci(); izracunajSveStatistike(); }, 500);", true);
        }
        ...
        private void BindEkipePoKategoriji(string kategorija)
        {
            ...
            using (SqlCommand cmd = new SqlCommand(query, con))
            {
                ...
                {
                    ddlFilterEkipaDvd.Items.Clear();
                    ddlFilterEkipaDvd.Items.Add(new ListItem("-- Изаберите екипу --", "0"));
                    ...
                    while (reader.Read())
                    {
                        ...
                        ddlFilterEkipaDvd.Items.Add(new ListItem($"{naziv} ({kat})", id));
                    }
                }
            }
        }

        private void UcitajPodatkeZaEkipuDobrovoljci(string ekipaID)
        {
            if (ekipaID == "0")
            {
                OcistiFormu();
            ...
            string connString = ConfigurationManager.ConnectionStrings["con"].ConnectionString;
            string query = @"SELECT TOP 1 * FROM dobrovoljciRezultati 
        ...
        protected void btnSacuvaj_Click(object sender, EventArgs e)
        {
            ...
            {
                string connString = ConfigurationManager.ConnectionStrings["con"].ConnectionString;
                string ekipaID = ddlFilterEkipaDvd.SelectedValue;
                ...
                using (SqlConnection con = new SqlConnection(connString))
                {
                    ...
                    UcitajPodatkeZaEkipuDobrovoljci(ekipaID);
                }
            }
            catch (Exception ex)
            {
        ...
        {
            string query = "SELECT COUNT(*) FROM dobrovoljciRezultati WHERE EkipaID = @EkipaID";
        ...
        {
            string query = @"INSERT INTO dobrovoljciRezultati 
        ...
        {
            string query = @"UPDATE dobrovoljciRezultati SET 
        ...
        {
            lblSrednjaVrednostDobrovoljciMvp.Text = "Средња време препреке: 0.00";
            lblSrednjaVrednostDobrovoljciStafeta.Text = "Средња вредност штафете: 0.00";
            lblZbirGresakaDobrovoljciMvp.Text = "Збир грешака препреке: 0.00";
            lblZbirGresakaDobrovoljciStafeta.Text = "Збир грешака штафете: 0.00";
            lblUkupanPlasmanDobrovoljci.Text = "Укупан резултат: 100.00";
        }
