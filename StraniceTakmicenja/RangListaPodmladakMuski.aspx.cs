...
{
    public partial class RangListaPodmladakMuski : System.Web.UI.Page
    {
        ...
        private void BindRangLista()
        {
            try
            {
                DataTable dt = UzmiRangListuMuski();

                if (dt.Rows.Count > 0)
                {
                    GridViewPodmladakMuski.DataSource = dt;
                    GridViewPodmladakMuski.DataBind();
                    ApplyRangColors();
                    ShowStatus($"Приказано {dt.Rows.Count} мушких екипа подмладка", "info");
                }
                else
                {
                    ShowStatus("Нема екипа у категорији 'Ватрогасни подмладак - мушка'.", "upozorenje");
                    GridViewPodmladakMuski.DataSource = null;
                    GridViewPodmladakMuski.DataBind();
                }
            }
            catch (Exception ex)
        ...
        private DataTable UzmiRangListuMuski()
        {
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                string query = @"
                    ...
                    WHERE e.Kategorija = N'Ватрогасни подмладак - мушка'
                    ORDER BY 
                        CASE 
                            WHEN pr.KonacniBodPodmladak IS NOT NULL AND pr.KonacniBodPodmladak > 0 
                            THEN pr.KonacniBodPodmladak 
        ...
        {
            foreach (GridViewRow row in GridViewPodmladakMuski.Rows)
            {
